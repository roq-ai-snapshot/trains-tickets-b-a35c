...
import { useRoqClient, useTrainFindFirst } from 'lib/roq';
...
function TrainEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTrainFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: TrainInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.train.update({
        data: values as any,
        where: {
          id,
        },
