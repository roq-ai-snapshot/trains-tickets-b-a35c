...
import { useRoqClient, useStationFindFirst } from 'lib/roq';
...
function StationEditPage() {
  ...
  const { data, error, isLoading, mutate } = useStationFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: StationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.station.update({
        data: values as any,
        where: {
          id,
        },
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <TextInput
            error={formik.errors.station_code}
            label={'Station Code'}
            props={{
              name: 'station_code',
