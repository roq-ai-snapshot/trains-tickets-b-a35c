...
import { useRoqClient, useStationFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<StationInterface, unknown>;
...
export function StationListPage(props: StationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: [, 'route.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: [
            'station_name.contains',
            'station_code.contains',
            'city.contains',
            'state.contains',
            'country.contains',
          ],
  ...
  const { data, error, isLoading, mutate } = useStationFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.station.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: StationInterface) => {
    if (hasAccess('station', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/stations/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'station_name', header: 'Station Name', accessorKey: 'station_name' },
    { id: 'station_code', header: 'Station Code', accessorKey: 'station_code' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/stations/view/${record.id}`} passHref legacyBehavior>
                <Button
