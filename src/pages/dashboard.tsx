...
import { useRouteCount, useTrainCount, useStationCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const trainStat = useTrainCount();

  const stationStat = useStationCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Trains',
      result: trainStat?.data,
      isLoading: trainStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Stations',
      result: stationStat?.data,
      isLoading: stationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
